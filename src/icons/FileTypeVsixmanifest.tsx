import type { SVGProps } from "react"
const FileTypeVsixmanifest = (props: SVGProps<SVGSVGElement>) => (
	<svg {...props} xmlns="http://www.w3.org/2000/svg" viewBox="0 0 32 32">
		{" "}
		<title>file_type_vsixmanifest</title> <path d="M2,16V30H15V24H8V8h7V2H2Z" style={{ fill: "#cfcfcf" }} />{" "}
		<path d="M17,5V8h4V6h5v5H24v4h6V2H17Z" style={{ fill: "#cfcfcf" }} />{" "}
		<path d="M10,16v6H22V10H10Z" style={{ fill: "#cfcfcf" }} />{" "}
		<path d="M24,20.5V24H17v6H30V17H24Z" style={{ fill: "#cfcfcf" }} />{" "}
		<path
			d="M26.078,26.787l.675-.67c3.123.1,3.17-.008,3.308-.329l.858-2.089.064-.2-.071-.173c-.036-.092-.149-.365-2.1-2.224v-.978c2.25-2.167,2.2-2.278,2.071-2.595l-.855-2.111c-.128-.316-.177-.44-3.277-.355l-.675-.7a15.169,15.169,0,0,0-.106-3.079l-.088-.2-2.23-.976c-.329-.149-.444-.2-2.55,2.089l-.947-.013c-2.162-2.315-2.266-2.276-2.59-2.142l-2.071.838c-.324.131-.439.179-.314,3.328l-.669.667c-3.121-.1-3.169.009-3.3.329L10.345,17.3l-.068.2.071.174c.036.09.144.36,2.1,2.221v.976c-2.25,2.167-2.2,2.278-2.071,2.6l.854,2.113c.13.323.177.438,3.276.357l.675.7a15.2,15.2,0,0,0,.1,3.075l.088.2,2.244.981c.327.137.439.185,2.539-2.1l.946.011c2.171,2.317,2.283,2.272,2.6,2.145l2.071-.837C26.1,29.984,26.209,29.932,26.078,26.787ZM17.18,21.9a3.718,3.718,0,1,1,4.866,2.118,3.76,3.76,0,0,1-4.866-2.119Z"
			style={{ fill: "#40535b" }}
		/>{" "}
	</svg>
)
export default FileTypeVsixmanifest
