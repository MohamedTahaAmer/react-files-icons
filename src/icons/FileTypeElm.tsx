import type { SVGProps } from "react"
const FileTypeElm = (props: SVGProps<SVGSVGElement>) => (
	<svg {...props} xmlns="http://www.w3.org/2000/svg" viewBox="0 0 32 32">
		{" "}
		<title>file_type_elm</title> <path d="M16,16.768,2.768,30H29.232Z" style={{ fill: "#5fb4cb" }} />{" "}
		<path d="M24.13,23.362,30,29.232V17.492Z" style={{ fill: "#eea400" }} />{" "}
		<path d="M15.232,16,2,2.768V29.232Z" style={{ fill: "#596277" }} />{" "}
		<path d="M30,14.448V2H17.552Z" style={{ fill: "#5fb4cb" }} />{" "}
		<path
			d="M23.392,9.376l6.594,6.594-6.624,6.624L16.768,16ZM2.768,2,8.863,8.1H22.111L16.016,2Z"
			style={{ fill: "#8cd636" }}
		/>{" "}
		<path d="M16,15.232l6.051-6.051H9.949Z" style={{ fill: "#eea400" }} />{" "}
	</svg>
)
export default FileTypeElm
