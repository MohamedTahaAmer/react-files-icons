import type { SVGProps } from "react"
const FileTypeBosque = (props: SVGProps<SVGSVGElement>) => (
	<svg {...props} xmlns="http://www.w3.org/2000/svg" viewBox="0 0 32 32">
		{" "}
		<title>file_type_bosque</title>{" "}
		<path d="M17.636,16.215v-6.1c0-.327-.054-.285.331-.285H30v6.379H24.66Z" style={{ fill: "#206e20" }} />{" "}
		<path d="M17.634,16.215H30v6.379H17.634V16.215Z" style={{ fill: "#247b24" }} />{" "}
		<path
			d="M29.949,3.813A1.188,1.188,0,0,0,28.859,3H10.22a1.181,1.181,0,0,0-1.2.939A.876.876,0,0,0,9,4.151V6.3H9V9.83H30V3.938C29.955,3.908,29.963,3.855,29.949,3.813Z"
			style={{ fill: "#1c601c" }}
		/>{" "}
		<path
			d="M19.7,22.592h-.2v0H9v3.533H9V27.9a.6.6,0,0,0,.013.112A1.173,1.173,0,0,0,10.2,29H28.786a1.171,1.171,0,0,0,1.17-.854c.01-.029,0-.069.044-.082V22.592Z"
			style={{ fill: "#288928" }}
		/>{" "}
		<path
			d="M18.322,9.07a1.223,1.223,0,0,0-.111-.508,1.32,1.32,0,0,0-1.263-.823Q12.973,7.73,9,7.735H3.387A1.361,1.361,0,0,0,2,9.069V22.853a1.363,1.363,0,0,0,1.313,1.411q.043,0,.087,0H16.839a2.241,2.241,0,0,0,.347-.024,1.35,1.35,0,0,0,1.14-1.307Q18.325,16,18.322,9.07Z"
			style={{ fill: "#107c10" }}
		/>{" "}
		<path
			d="M13.785,21.26H12.546c-.084,0-.081-.069-.1-.116q-.524-1.4-1.043-2.8l-.954-2.557c-.011-.029-.024-.059-.035-.088-.056.008-.054.055-.068.085Q9.121,18.461,7.9,21.134a.18.18,0,0,1-.2.129c-.381-.008-.764,0-1.163,0,.175-.39.344-.767.514-1.143q1.353-3,2.706-5.991a.251.251,0,0,0,.006-.214c-.155-.41-.3-.822-.457-1.233a1.719,1.719,0,0,0-.219-.426A.931.931,0,0,0,8.3,11.84H7.769c-.088,0-.123-.015-.121-.115.008-.284,0-.567,0-.851,0-.049-.016-.105.07-.1a6.866,6.866,0,0,1,1.366.089,1.708,1.708,0,0,1,1.289,1.155c.691,1.891,1.392,3.778,2.089,5.666l1.269,3.436A1.123,1.123,0,0,1,13.785,21.26Z"
			style={{ fill: "#fefefe" }}
		/>{" "}
	</svg>
)
export default FileTypeBosque
