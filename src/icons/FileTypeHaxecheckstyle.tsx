import type { SVGProps } from "react"
const FileTypeHaxecheckstyle = (props: SVGProps<SVGSVGElement>) => (
	<svg {...props} xmlns="http://www.w3.org/2000/svg" viewBox="0 0 32 32">
		{" "}
		<title>file_type_haxecheckstyle</title>{" "}
		<polygon
			points="26.034 7.613 24.005 7.613 24.005 21.34 26.034 21.34 26.034 7.613 26.034 7.613 26.034 7.613"
			style={{ fill: "#ffed00" }}
		/>{" "}
		<polygon points="30 7.613 26.034 7.613 26.034 21.34 30 21.34 30 7.613 30 7.613" style={{ fill: "#fcc204" }} />{" "}
		<path
			d="M30,21.34l-2.973,3.838L24.005,21.34Zm-4.121,0-.845-.661-.845.661Zm2,0-.845-.661-.845.661Zm2,0-.845-.661-.845.661Z"
			style={{ fill: "#fff7ad" }}
		/>{" "}
		<polygon points="25.818 23.643 27.025 25.178 28.212 23.643 25.818 23.643" style={{ fill: "#e53524" }} />{" "}
		<polygon
			points="26.034 3.391 24.005 3.391 24.005 7.245 26.034 7.245 26.034 3.391 26.034 3.391 26.034 3.391"
			style={{ fill: "#f7c3c6" }}
		/>{" "}
		<polygon points="30 3.391 26.034 3.391 26.034 7.245 30 7.245 30 3.391 30 3.391" style={{ fill: "#f5b2b6" }} />{" "}
		<path d="M12.324,5.972,4.581,13.715l7.743,7.743,7.743-7.743L12.324,5.972" style={{ fill: "#f68712" }} />{" "}
		<path d="M2,3.391,12.324,5.972,4.581,13.715,2,3.391" style={{ fill: "#fab20b" }} />{" "}
		<path d="M22.648,3.391,20.067,13.715,12.324,5.972,22.648,3.391" style={{ fill: "#f47216" }} />{" "}
		<path d="M22.648,24.039,12.324,21.458l7.743-7.743,2.581,10.324" style={{ fill: "#f25c19" }} />{" "}
		<path d="M2,24.039,4.581,13.715l7.743,7.743L2,24.039" style={{ fill: "#f89c0e" }} />{" "}
		<path d="M12.324,5.972,2,3.391H7.162l5.162,2.581" style={{ fill: "#fbc707" }} />{" "}
		<path d="M12.324,5.972,22.648,3.391H17.486L12.324,5.972" style={{ fill: "#fbc707" }} />{" "}
		<path d="M12.324,21.458l10.324,2.581H17.486l-5.162-2.581" style={{ fill: "#f68712" }} />{" "}
		<path d="M12.324,21.458,2,24.039H7.162l5.162-2.581" style={{ fill: "#f25c19" }} />{" "}
		<path d="M4.581,13.715,2,24.039V18.877l2.581-5.162" style={{ fill: "#fff200" }} />{" "}
		<path d="M4.581,13.715,2,3.391V8.553l2.581,5.162" style={{ fill: "#fff200" }} />{" "}
		<path d="M20.067,13.715,22.648,3.391V8.553l-2.581,5.162" style={{ fill: "#f1471d" }} />{" "}
		<path d="M20.067,13.715l2.581,10.324V18.877l-2.581-5.162" style={{ fill: "#f1471d" }} />{" "}
		<path
			d="M25.079,25.254a3.272,3.272,0,0,0-.694.538c-.2.2-.385.419-.56.628A2.96,2.96,0,0,1,22.6,27.462a1.92,1.92,0,0,1-2-.7c-.093-.1-.184-.213-.275-.322a3.669,3.669,0,0,0-1.963-1.456,3.017,3.017,0,0,0-2.949,1.191l-.169.2a2.716,2.716,0,0,1-1.593,1.177c-.621.07-1.186-.244-1.891-1.05a3.54,3.54,0,0,0-2.608-1.58H9.134a3.652,3.652,0,0,0-2.648,1.622c-.679.776-1.289,1.092-1.92.993a2.355,2.355,0,0,1-1.327-.924L2,26.775c.064.074.127.15.191.226A3.737,3.737,0,0,0,4.4,28.573a3.128,3.128,0,0,0,2.869-1.338c.567-.648,1.1-1.26,1.867-1.265h.009c.7,0,1.2.507,1.821,1.222a3.484,3.484,0,0,0,2.517,1.417,2.5,2.5,0,0,0,.28-.016,3.577,3.577,0,0,0,2.272-1.538l.164-.191a2.055,2.055,0,0,1,1.931-.855,2.8,2.8,0,0,1,1.388,1.107c.1.116.195.232.294.343a2.892,2.892,0,0,0,3.122.993,3.892,3.892,0,0,0,1.693-1.361c.167-.2.325-.388.494-.556a2.247,2.247,0,0,1,.471-.37Z"
			style={{ fill: "#e53524" }}
		/>{" "}
	</svg>
)
export default FileTypeHaxecheckstyle
