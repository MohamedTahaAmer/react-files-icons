import type { SVGProps } from "react"
const FolderTypeInclude = (props: SVGProps<SVGSVGElement>) => (
	<svg {...props} xmlns="http://www.w3.org/2000/svg" viewBox="0 0 32 32">
		{" "}
		<title>folder_type_include</title>{" "}
		<path d="M27.5,5.5H18.2L16.1,9.7H4.4V26.5H29.6V5.5Zm0,4.2H19.3l1.1-2.1h7.1Z" style={{ fill: "#686868" }} />{" "}
		<path
			d="M15.266,11.311V29.686H29.715v-15.1h-3.6V11.311Zm12.146.813.008,1.158h1.252l-1.26-1.16Z"
			style={{ fill: "gray" }}
		/>{" "}
		<path
			d="M27.028,10H15.281a1.316,1.316,0,0,0-1.312,1.313V29.688A1.316,1.316,0,0,0,15.281,31H29.719a1.316,1.316,0,0,0,1.313-1.312V13.642Zm.394,2.133,1.247,1.148H27.422ZM15.281,29.688V11.313H26.109v3.281h3.609V29.688Z"
			style={{ fill: "#c5c5c5" }}
		/>{" "}
		<polygon
			points="17.523 27.326 23.1 21.749 17.523 16.172 17.523 19.661 10.1 19.661 10.1 23.837 17.523 23.837 17.523 27.326"
			style={{ fill: "#c5c5c5" }}
		/>{" "}
	</svg>
)
export default FolderTypeInclude
