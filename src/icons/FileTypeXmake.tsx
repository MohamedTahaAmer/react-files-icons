import type { SVGProps } from "react"
const FileTypeXmake = (props: SVGProps<SVGSVGElement>) => (
	<svg {...props} xmlns="http://www.w3.org/2000/svg" viewBox="0 0 32 32">
		{" "}
		<title>file_type_xmake</title> <circle cx="16" cy="16" r="14" style={{ fill: "#e0f2f1" }} />{" "}
		<path d="M23.518,5.888A12.6,12.6,0,0,0,5.654,8.811l6.562,4.047Z" style={{ fill: "#e0f2f1" }} />{" "}
		<path
			d="M3.591,18.177l8.625-5.319L5.654,8.811A12.543,12.543,0,0,0,3.4,16,12.7,12.7,0,0,0,3.591,18.177Z"
			style={{ fill: "#8bc34a" }}
		/>{" "}
		<path d="M27.072,22.019A12.607,12.607,0,0,0,23.518,5.888l-11.3,6.97Z" style={{ fill: "#3dafa4" }} />{" "}
		<path d="M3.591,18.177a12.6,12.6,0,0,0,23.481,3.842L12.216,12.858Z" style={{ fill: "#22a079" }} />{" "}
	</svg>
)
export default FileTypeXmake
